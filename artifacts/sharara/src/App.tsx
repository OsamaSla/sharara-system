...
import './App.css';
import { Trash2, FileSpreadsheet, Layers, CreditCard, Building2, Briefcase, User, Phone, Mail, CheckCircle2, FileDown, Upload, X } from 'lucide-react';
...
export default function App() {
  ...
  // לוגו החברה — נשמר כ-base64 data URL ב-localStorage
  const [logoDataUrl, setLogoDataUrl] = useState<string>(() => {
    return localStorage.getItem('sharara_logoDataUrl') || '';
  });
  ...
  useEffect(() => {
    if (logoDataUrl) {
      localStorage.setItem('sharara_logoDataUrl', logoDataUrl);
    } else {
      localStorage.removeItem('sharara_logoDataUrl');
    }
  }, [logoDataUrl]);
  ...
  const calculateArea = (row: RowData): number => {
    const { type, width1, height1, width2, height2, length, rBig, rSmall, notes, rBig2 } = row;
    ...
    if (notes === 'לאמד S') {
      ...
      return (2 * (width1 + height1) * totalLen) / 1000000;
    }
      ...
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#0f172a', fontFamily: 'Assistant, Rubik, sans-serif', direction: 'rtl', padding: '20px', boxSizing: 'border-box' }}>
        <div style={{ backgroundColor: '#ffffff', borderRadius: '12px', border: '1px solid #334155', padding: '40px 32px', width: '100%', maxWidth: '420px', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)', display: 'flex', flexDirection: 'column', alignItems: 'center', boxSizing: 'border-box' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', marginBottom: '24px', textAlign: 'center' }}>
            <img src="/logo.png" alt="לוגו עלי שרארה" style={{ maxHeight: '80px', width: 'auto', objectFit: 'contain', marginBottom: '4px' }} />
            <h1 style={{ fontSize: '26px', fontWeight: '950', color: '#0f172a', margin: '0', fontFamily: 'Rubik, sans-serif' }}>עלי שרארה בע"מ</h1>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '15px' }}>
            <div style={{ position: 'relative' }}>
              {logoDataUrl ? (
                ...
                <div style={{ backgroundColor: '#ffffff', borderRadius: '8px', padding: '8px', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.3)', border: '1px solid #334155' }}>
                  <img src="/logo.png" alt="לוגו החברה" style={{ maxHeight: '80px', width: 'auto', objectFit: 'contain' }} />
                </div>
              )}
            </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 1fr', gap: '20px' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', backgroundColor: '#ffffff', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                        ...
                        <div style={{ marginBottom: '8px' }}>
                          ...
                          <input type="text" value={newPartData.partNumber} onChange={(e) => setNewPartData({...newPartData, partNumber: e.target.value})} style={{ width: '100%', padding: '6px 10px', border: '1px solid #cbd5e1', borderRadius: '4px', backgroundColor: '#ffffff', color: '#0f172a' }} placeholder="לדוגמה: P001" />
                        </div>
                        ...
                        {newPartData.notes === 'צינור עגול' ? (
                          ...
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                            ...
                            <div>
                              ...
                              <input type="number" value={newPartData.rBig2 || ''} onChange={(e) => setNewPartData({...newPartData, rBig2: Number(e.target.value)})} style={{ width: '100%', padding: '6px 10px', border: '1px solid #cbd5e1', borderRadius: '4px', backgroundColor: '#ffffff', color: '#0f172a' }} />
                            </div>
                          </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', backgroundColor: '#ffffff', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '10px' }}>
                          ...
                          <div>
                            ...
                            <input type="number" value={newPartData.flexible || ''} onChange={(e) => setNewPartData({...newPartData, flexible: Number(e.target.value)})} style={{ width: '100%', padding: '6px 10px', border: '1px solid #cbd5e1', borderRadius: '4px', backgroundColor: '#ffffff', color: '#0f172a' }} />
                          </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', backgroundColor: '#ffffff', padding: '16px', borderRadius: '8px', border: '1px solid #e2e8f0' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '8px' }}>
                          <div>
                            ...
                            <select value={newPartData.sharshuriType} onChange={(e) => setNewPartData({...newPartData, sharshuriType: e.target.value as any})} style={{ width: '100%', padding: '6px 10px', border: '1px solid #cbd5e1', borderRadius: '4px', backgroundColor: '#ffffff', color: '#0f172a', fontWeight: '600' }}>
                          ...
                          <div>
                            ...
                            <input type="number" disabled={newPartData.adapterType === 'ללא'} value={newPartData.adapterQty || ''} onChange={(e) => setNewPartData({...newPartData, adapterQty: Number(e.target.value)})} style={{ width: '100%', padding: '6px 10px', border: '1px solid #cbd5e1', borderRadius: '4px', backgroundColor: newPartData.adapterType === 'ללא' ? '#e2e8f0' : '#ffffff', color: '#0f172a' }} />
                          </div>

                          {/* דופן - תוסף לכל סוג חלק */}
                          <div style={{ gridColumn: 'span 2', display: 'flex', gap: '12px', alignItems: 'center', padding: '10px 12px', backgroundColor: '#fefce8', borderRadius: '6px', border: '1px solid #facc15' }}>
                            <label style={{ fontSize: '11px', fontWeight: 'bold', color: '#854d0e', display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
                              <input type="checkbox" checked={newPartData.panels > 0} onChange={(e) => setNewPartData({...newPartData, panels: e.target.checked ? 1 : 0})} style={{ width: '16px', height: '16px', cursor: 'pointer' }} />
                              דופן
                            </label>
                            {newPartData.panels > 0 && (
                              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                <span style={{ fontSize: '11px', fontWeight: 'bold', color: '#854d0e' }}>כמות:</span>
                                <input type="number" min="1" value={newPartData.panels} onChange={(e) => setNewPartData({...newPartData, panels: Math.max(1, Number(e.target.value))})} style={{ width: '60px', padding: '4px 8px', border: '1px solid #facc15', borderRadius: '4px', backgroundColor: '#ffffff', color: '#0f172a', fontWeight: 'bold', textAlign: 'center' }} />
                              </div>
                            )}
                          </div>
                          ...
                          <div style={{ gridColumn: 'span 2', marginTop: '4px' }}>
                            ...
                            <input type="text" value={newPartData.notes} onChange={(e) => setNewPartData({...newPartData, notes: e.target.value})} style={{ width: '100%', padding: '6px 10px', border: '1px solid #cbd5e1', borderRadius: '4px', backgroundColor: '#ffffff', color: '#0f172a' }} placeholder="כתוב הערה..." />
                          </div>
                        </div>
                      </div>
                      ...
                      {activeSheet.rows.map((row, idx) => (
                        <tr key={row.id} style={{ borderBottom: '1px solid #e2e8f0' }}>
                          ...
                          <td style={{ padding: '8px', textAlign: 'center', backgroundColor: '#fefce8' }}>
                            <input type="number" min="0" value={row.panels || ''} onChange={(e) => updateRow(row.id, 'panels', Math.max(0, Number(e.target.value)))} style={{ width: '40px', padding: '4px', textAlign: 'center', border: '1px solid #facc15', borderRadius: '4px', backgroundColor: row.panels > 0 ? '#fffbeb' : '#ffffff', color: '#854d0e', fontWeight: row.panels > 0 ? 700 : 400 }} placeholder="0" />
                          </td>
            ...
            {activeTab === 'summary' && (
              <div className="landscape-print" style={{ backgroundColor: '#ffffff', borderRadius: '8px', border: '1px solid #cbd5e1', padding: '32px', maxWidth: '1400px', margin: '0 auto', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.05)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', borderBottom: '3px double #0f172a', paddingBottom: '15px', marginBottom: '20px' }}>
                  <img src={logoDataUrl || "/logo.png"} alt="לוגו" style={{ maxHeight: '70px', width: 'auto', objectFit: 'contain', marginBottom: '6px' }} />
              ...
              <div className="portrait-print" style={{ backgroundColor: '#ffffff', borderRadius: '8px', border: '1px solid #cbd5e1', padding: '32px', maxWidth: '750px', margin: '0 auto', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.05)' }}>
                ...
                {isEditingMyCompany && (
                  <div className="no-print" style={{ backgroundColor: '#f8fafc', border: '1px solid #cbd5e1', borderRadius: '8px', padding: '20px', marginBottom: '24px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
                      ...
                      <div style={{ gridColumn: 'span 2' }}>
                        <label style={{ fontSize: '11px', fontWeight: 'bold', color: '#475569', display: 'block', marginBottom: '6px' }}>📷 לוגו החברה (PNG / JPG / SVG):</label>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                          {logoDataUrl && (
                            <img src={logoDataUrl} alt="לוגו" style={{ maxHeight: '60px', maxWidth: '120px', objectFit: 'contain', border: '1px solid #cbd5e1', borderRadius: '4px', background: '#f8fafc', padding: '4px' }} />
                          )}
                          <button
                            type="button"
                            onClick={() => logoInputRef.current?.click()}
                            style={{ padding: '7px 14px', backgroundColor: '#2563eb', color: '#ffffff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '5px' }}
                          >
                            <Upload size={13} /> {logoDataUrl ? 'החלף לוגו' : 'העלה לוגו'}
                          </button>
                          {logoDataUrl && (
                            <button
                              type="button"
                              onClick={() => setLogoDataUrl('')}
                              style={{ padding: '7px 12px', backgroundColor: '#dc2626', color: '#ffffff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '12px', display: 'flex', alignItems: 'center', gap: '4px' }}
                            >
                              <X size={13} /> הסר
                            </button>
                          )}
                          ...
                          />
                          <span style={{ fontSize: '11px', color: '#64748b' }}>הלוגו ישמר אוטומטית וישולב בכל הדוחות</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', borderBottom: '3px double #0f172a', paddingBottom: '20px', marginBottom: '24px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', textAlign: 'center', marginBottom: '10px' }}>
                    <img src={logoDataUrl || "/logo.png"} alt="לוגו" style={{ maxHeight: '90px', width: 'auto', objectFit: 'contain', marginBottom: '8px' }} />
                    ...
                    >
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', borderBottom: '3px solid #0f172a', paddingBottom: '15px', marginBottom: '25px', position: 'relative' }}>
                        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '20px', width: '100%' }}>
                          <img src={logoDataUrl || "/logo.png"} alt="לוגו" style={{ maxHeight: '55px', width: 'auto', objectFit: 'contain' }} />
                        </div>
